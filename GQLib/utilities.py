...
def generate_all_dates_sp(frequency: str = "daily",
                       lppl_model: 'LPPL | LPPLS' = LPPL,
                       optimizers: list = [SA(), SGA(), PSO(), MPGA()],
                       nb_tc: int = None,
                       significativity_tc=0.3,
                       save: bool = False):
    if frequency not in ["daily", "weekly", "monthly"]:
    ...
    # Initialisation des dates
    dates_sets = {
        "Set 1": ("01/11/1990", "01/02/2000"),
        "Set 2": ("01/12/2001", "01/01/2007"),
        "Set 3": ("01/07/2009", "01/09/2019"),
    }
    dates_graphs = [
        ("01/11/1990", "01/02/2010"),
        ("01/12/2001", "01/01/2017"),
        ("01/07/2009", "01/09/2025"),
    ]
    ...
    for optimizer in optimizers:
        ...
        for set_name, (start_date, end_date) in dates_sets.items():
            ...
            start_date_obj = datetime.strptime(start_date, "%d/%m/%Y")
            end_date_obj = datetime.strptime(end_date, "%d/%m/%Y")
            filename = f"results/{optimizer.__class__.__name__}/{frequency}/{lppl_model.__name__}_{start_date_obj.strftime('%m-%Y')}_{end_date_obj.strftime('%m-%Y')}.json"
...
def generate_all_rectangle_sp(frequency: str = "daily",
                           lppl_model: 'LPPL | LPPLS' = LPPL,
                           optimizers: list = [SA(), SGA(), PSO(), MPGA()],
                           significativity_tc=0.3,
                           rerun: bool = False,
    ...
    for set_name, (start_date, end_date) in dates_sets.items():
        ...
        for optimizer in optimizers:
            ...
            start_date_obj = datetime.strptime(start_date, "%d/%m/%Y")
            end_date_obj = datetime.strptime(end_date, "%d/%m/%Y")
            filename = f"results/SP500/{optimizer.__class__.__name__}/{frequency}/{lppl_model.__name__}/{start_date_obj.strftime('%m-%Y')}_{end_date_obj.strftime('%m-%Y')}.json"
        ...
        fw.compare_results_rectangle(multiple_results=best_results_list, name="Predicted LPPL critical times",
                                     data_name="SP 500", real_tc=real_tcs[compteur], start_date=start_date,
                                     end_date=end_date)
        compteur += 1